...
- **Upload panel** — drag-and-drop or click-to-browse resume upload, posts
  directly to `POST /api/upload/resume`
- **Live stats** — resumes processed, average extraction accuracy, and profiles
  created, computed client-side from `GET /api/candidates/`
- **Most recent extraction** — shows the newest candidate's parsed fields
  (name, email, phone, experience, skills) right after upload
- **Candidate table** — all processed candidates with key skills and a delete
  action per row (`DELETE /api/candidates/{id}`)
- **Export CSV** button — triggers `GET /api/candidates/export/csv` as a
  browser download

No React/Vue/build tooling is used intentionally, to keep Milestone 1 easy to
run with zero extra setup. If the project grows a more complex UI in a later
milestone, this would be a natural point to migrate to a proper frontend
framework.
